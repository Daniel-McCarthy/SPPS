###############################################################################################################
# Thanks and credit to the Resident Evil Code Veronica X decompilation project for creating the original script
# Their project can be found here: https://github.com/fmil95/recvx-decomp/
###############################################################################################################
import io
import struct
import argparse
...
def get_arguments(argv=None):
    ...
    parser.add_argument(
        "--output_filename",
        required=False,
        help="optional name to override the default name of the created iso file for insert mode",
    )

    args = parser.parse_args()
    curr_dir = Path("./").resolve()
    output_filename = args.output_filename if args.output_filename else "SPPS_NEW.iso"
    ...
    args.filelist = curr_dir / "iso" / "sort.txt"
    args.files = curr_dir / "iso" / "extract"
    args.output = curr_dir / "iso" / output_filename
