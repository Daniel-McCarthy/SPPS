...
Event* event, signed int type, signed int no, signed int id, signed int player);
...
void tmevGetLightVector(sceVu0FMATRIX light_color, sceVu0FMATRIX normal_light, float* ambient, signed int player);
...
s32 tmevReset(Event* event) {
    signed int ii; // r16
    unsigned int time_tbl[8] = {
        0x2BF20, 0x2BF20, 0x33450, 0x33450,
        0x3A980, 0x3A980, 0x493E0, 0x493E0
    }; // 0x50(r29)
    unsigned int* timetbl_ptr = time_tbl;
    unsigned int vs_time[8] = {
        0x50910, 0x493E0, 0x493E0, 0x4E200,
        0x51C98, 0x53020, 0x57E40, 0x4E200
    }; // 0x70(r29)
...
s32 tmevMainEvent(Event* event) {
    ...
    tmcrsSetBGColor(vtmevCourse[id].bg_col[0], vtmevCourse[id].bg_col[1], vtmevCourse[id].bg_col[2]);
...
s32 tmevPlayerEvent(Event* event) {
    ...
    if ((event->character[player].old_rail != -1) && (event->character[player].ctrl->act.sliding_state != essPlant) && (event->character[player].ctrl->act.sliding_state != essGrind) && (event->character[player].ctrl->act.sliding_state != essStandUp) && (event->character[player].ctrl->act.sliding_state != 3)) {
    ...
    if ((event->character[player].ctrl->act.sliding_state == essGrind) && (event->character[player].old_rail == -1)) {
        event->character[player].old_rail = event->character[player].rail;
    }
    if ((event->character[player].ctrl->act.sliding_state == essGrind) && (event->character[player].old_rail != event->character[player].rail)) {
        event->character[player].old_rail = event->character[player].rail;
    }
...
s32 tmevFinishPlayerEvent(Event* event) {
    ...
    if (event->game.mode != 2) {
        ...
        if (event->game.nplayer == 1) {
            ...
            if ((time >= vsptblLevelGoalValue[event->course.no][3]) && (score >= vsptblLevelGoalValue[event->course.no][4]) && (event->character[player].ctrl->act.pass_finish_line == 1)) {
                tmevSetLevelGoal(player, 3);
            }
            if (tmp < 12) {
                level_goal = vspenvSecret->character[tmp].level_goal[event->course.no] | tmevGetLevelGoal(player);
            } else {
                level_goal = vspenvSecret->create_character[tmp - 12].character.level_goal[event->course.no] | tmevGetLevelGoal(player);
            }
...
static void tmevInitSystem(Event* event) {
    ...
    for (ii = 0; ii < event->game.nplayer; ii++) {
        ...
        vtmevSystem.level_goals_tmp[ii] = vspenvReplay[ii]->character.character.level_goal[vtmevSystem.crs_no];
...
static void tmevResetSystem(Event* event) {
    ...
    if (event->game.mode == 2) {
        ...
        for (ii = 0; ii < 2; ii++) {
            vtmevSystem.old_level_goals[ii] = vtmevSystem.level_goals_tmp[ii];
            vspenvReplay[ii]->character.character.level_goal[vtmevSystem.crs_no] = vtmevSystem.level_goals_tmp[ii];
        }
    }
}
...
static void tmevCheckLevelGoal(Event* event /* sp70 */) {
    ...
    score = event->character[player].ctrl->act.trick_link.total_trick_point;
    for (ii = 0; ii < 3; ii++) {
        if (score >= vsptblLevelGoalValue[event->course.no][ii] && ((1 << ii) & tmevGetLevelGoal(player)) == 0) {
    ...
    if (tmp < 0xC) {
        ...
        secret = &((Create_Character*)vspenvSecret)[tmp - 12].character.secret + event->course.no;
        level_goal = (tmevGetLevelGoal(player) | secret->character[12].level_goal[0]);
    }
...
s32 tmevGetLevelGoalFlag(s32 no) {
    ...
    ret = 0;
    if (((vspenvGame->mode.game_mode == 0) || (vspenvGame->mode.game_mode == 2)) && (vtmevSystem.level_goals_tmp[0] & (1 << no))) {
        ret = 1;
    }
    return ret;
}
...
s32 tmevGetICounter(s32 no) {
    return vtmevCounter.i_cnt[no];
}
...
void tmevSetFlag(s32 no, s32 value) {
    vtmevFlag[no] = value;
}

s32 tmevGetFlag(s32 no) {
    return vtmevFlag[no];
}

void tmevSetFog(s32 fog, s32 player) {
    vtmevCourse[player].fog = fog;
}

void tmevSetBGFog(s32 fog, s32 player) {
    vtmevCourse[player].bg_fog = fog;
}

void tmevSetBGNo(s32 no, s32 player) {
    vtmevCourse[player].bg_no = no;
}
...
f32 tmevGetDrawLength(s32 player) {
    return vtmevCourse[player].draw_length;
}
...
void tmevMoveObject(Event* event) {
    ...
    for (ii = 0; ii < event->character[0].nobj; ii++) {
        for (jj = 0; jj < vtmevObject.logo.nobj; jj++) {
            if ((vtmevObject.logo.obj_no[jj] == event->character[0].object[ii].obj_no) && (tmcrsGetObjectDrawFlag(vtmevObject.logo.obj_no[jj]) != 0)) {
        ...
        for (jj = 0; jj < 0x20; jj++) {
            if ((vtmevObject.sponsor.obj_no[ii] != -1) && (vtmevObject.sponsor.obj_no[jj] == event->character[0].object[ii].obj_no)) {
    ...
    for (ii = 0; ii < 0x20; ii++) {
        if ((vtmevObject.sponsor.obj_no[ii] != -1) && (tmcrsGetObjectDrawFlag(vtmevObject.sponsor.obj_no[ii]) != 0)) {
...
static void tmevResetSponsor() {
    ...
    if ((vtmevSystem.level_goals_tmp[0] & 0x20) == 0 && (vspenvGame->mode.game_mode == 0)) {
...
static void tmevRollSponsor() {
    ...
    trans[3] = 1.0f;
    for (ii = 0; ii < 0x20; ii++) {
        if ((vtmevObject.sponsor.obj_no[ii] != -1) && (tmcrsGetObjectDrawFlag(vtmevObject.sponsor.obj_no[ii]) == 1)) {
    ...
    for (ii = 0; ii < event->character[0].nhit; ii++) {
        for (jj = 0; jj < vtmevWarpSystem.nwarp; jj++) {
            if (vtmevWarpSystem.warp[jj].hit_no == event->character[0].hit[ii].no) {
                ktactSetRecover(0, &vtmevWarpSystem.warp[jj].rec_pos[0], vtmevWarpSystem.warp[jj].angle, vtmevWarpSystem.warp[jj].speed, 1);
...
s32 tmevGetNowHorsePos() {
    return vtmevVs.now_pos;
}

s32 tmevGetLap(signed int player /* r29 */) {
    return vtmevVs.lap[player];
}
...
static void tmevCheckVsWarp(Event* event) {
    ...
    tmp[3] = 1.0f;
    if ((vtmevVs.now_warp[player] == 1) && event->character[player].ctrl->act.trg_recovered == 1) {
        if (vspenvGame->mode.match_rule == 1) {
            vtmevVs.lap[player]++;
        }
...
s32 tmevGetFinish(signed int player) {
    return vtmevSystem.finish[player];
}
...
s32 tmevDefResetEvent(Event* event) {
    return 0;
}

s32 tmevDefMainEvent(Event* event) {
    return 0;
}

s32 tmevDefPlayerEvent(Event* event) {
    return 0;
}
