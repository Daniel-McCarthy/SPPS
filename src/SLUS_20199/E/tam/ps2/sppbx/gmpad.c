...
signed int sceGsSyncV(signed int mode);
void ulpadInit(signed int port_max, signed int slot_max);
void ulpadSetDualPause(signed int port, signed int slot, signed int id, signed int pause);
void ulpadStopDual(signed int port, signed int slot, signed int id);
void ulpadSetAna(signed int port, signed int slot, signed int ana, signed int lock);
signed int ulpadGet(VgmsysPadInfo* info, signed int port, signed int slot);
void ulpadUpdateDual();
    ...
    if (vgmsysPadPausePid < 0) {
        ...
        for (i = 0; i < num_player; i++) {
            if (vgmsysPadAllowPause & (1 << i)) {
                if ((vgmsysPad[i]->trg & 0x800) || (vgmsysPad[i] == &vgmsysPadNull)) {
                    pid = i;
                }
            }
        }
