...
// MWCC emits one section per symbol and reverses them, so these are declared in
...
signed int ulmcServer()
{
    ...
    if (vulmcSysInfo.fstat) {
        ...
        if (vulmcSysInfo.actSlot != -1) {
            vulmcSysInfo.reqdata = &vulmcCardData[vulmcSysInfo.actSlot];
            info = &vulmcSysInfo.reqdata->info;
            ...
            case 13:
                ...
                break;
            case 14:
                req = sceMcSetFileInfo((vulmcSysInfo.actSlot < 4) ^ 1, vulmcSysInfo.slot & 3, vulmcSysInfo.reqdata->name, (char*)&vulmcSysInfo.reqdata->newinfo, vulmcSysInfo.reqdata->valid);
                break;
            case 15:
            case 35:
...
signed int ulmcMkDir(signed int slot, char* dirname)
{
    if (vulmcSysInfo.slot == -1) {
        ...
        ulstdSprintf(vulmcCardData[slot].name, "%s", dirname);
        vulmcSysInfo.slot = slot;
        return slot;
    }
    return -1;
...
signed int ulmcGetDir(signed int slot, Table* table, char* filename, signed int mode, signed int max)
{
    if (vulmcSysInfo.slot == -1) {
        ...
        ulstdSprintf(vulmcCardData[slot].name, "%s", filename);
...
signed int ulmcDeleteAll(signed int slot)
{
    if (vulmcSysInfo.slot == -1) {
        slot = ulmcCheckSlotNo(slot);
        ulmcGetDir(slot, &vulmcDir[0], "*", 0, 0x14);
        vulmcCardData[slot].fnum[0] = 0xD;
